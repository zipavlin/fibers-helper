...
class Editor
{
    ...
    public function __construct(InputInterface $input, OutputInterface $output, $stream, $options = [])
    {
        $this->input = $input;
        $this->output = $output;
        $this->stream = $stream;
    ...
    {
        // remove char before cursor if possible
        if ($this->x > 0) {
            $this->c[$this->y] = substr($this->c[$this->y], 0, $this->x - 1) . substr($this->c[$this->y], $this->x);
            $this->left(true);
            $this->left(true);
            $this->writeRow($this->y, true);
        }
    }
    ...
    {
        if ($this->x < $this->length()) {
            $this->c[$this->y] = substr($this->c[$this->y], 0, $this->x) . substr($this->c[$this->y], $this->x + 1);
            $this->left(true);
            $this->writeRow($this->y, true);
        }
    }
    ...
    private function char ($char): void
    {
        if (!isset($this->c[$this->y])) $this->c[$this->y] = "";
        // add char if cursor is at the end of line
        if ($this->x === $this->length()) {
            $this->x++;
            $this->c[$this->y] .= $char;
            $this->output->write($char);
        }
        // add char in x position of string
        else {
            $this->c[$this->y] = substr($this->c[$this->y], 0, $this->x) . $char . substr($this->c[$this->y], $this->x);
            $this->writeRow($this->y, true);
        }
    }
